use super::NaiveTime;
use crate::{OldTimeDelta, Timelike, TimeDelta, Days};
...
#[test]
fn test_time_overflowing_add_old() {
    let hmsm = NaiveTime::from_hms_milli;
...
fn test_time_overflowing_add() {
    ...
    assert_eq!(
        hmsm(3, 4, 5, 678).checked_overflowing_add(Duration::from_secs(11 * 60 * 60)),
        Some((hmsm(14, 4, 5, 678), Days::new(0)))
    );
    assert_eq!(
        hmsm(3, 4, 5, 678).checked_overflowing_add(Duration::from_secs(23 * 60 * 60)),
        Some((hmsm(2, 4, 5, 678), Days::new(1)))
    );
    // overflowing_add_signed with leap seconds may be counter-intuitive
    assert_eq!(
        hmsm(3, 4, 5, 1_678).checked_overflowing_add(Duration::from_secs(24 * 60 * 60)),
...
fn test_time_overflowing_sub() {
    ...
    assert_eq!(
        hmsm(3, 4, 5, 678).checked_overflowing_sub(Duration::from_secs(7)),
        Some((hmsm(3, 3, 58, 678), Days::new(0)))
    );

    assert_eq!(
        hmsm(3, 4, 5, 1_678).checked_overflowing_sub(Duration::from_secs(24 * 60 * 60)),
        Some((hmsm(3, 4, 6, 678), Days::new(1)))
    );
}

#[test]
fn test_time_addassignment_old() {
...
#[test]
fn test_time_subassignment_old() {
    let hms = NaiveTime::from_hms;
    let mut time = hms(12, 12, 12);
    time -= OldTimeDelta::hours(10);
