...
impl NaiveTime {
    ...
    /// let from_hms = NaiveTime::from_hms;
    ///
    /// assert_eq!(from_hms(3, 4, 5).checked_overflowing_add(Duration::from_secs(60 * 60 * 11)),
    ...
    pub fn checked_overflowing_add(&self, rhs: Duration) -> Option<(NaiveTime, Days)> {
        ...
        Some((
            ...
            Days::new(u32::try_from(total_days).ok()?),
        ))
    }
    ...
    /// let from_hms = NaiveTime::from_hms;
    ///
    /// assert_eq!(from_hms(3, 4, 5).checked_overflowing_sub(Duration::from_secs(60 * 60 * 2)),
    ///            Some((from_hms(1, 4, 5), Days::new(0))));
    /// assert_eq!(from_hms(3, 4, 5).checked_overflowing_sub(Duration::from_secs(60 * 60 * 17)),
    ///            Some((from_hms(10, 4, 5), Days::new(1))));
    /// ```
    #[inline]
    pub fn checked_overflowing_sub(&self, rhs: Duration) -> Option<(NaiveTime, Days)> {
        ...
        if total_backwards_seconds > secs {
            ...
            let total_days = 1 + u32::try_from(backwards_secs_from_0 / 86_400).ok()?;
    ...
    ///
    /// assert_eq!(since(from_hmsm(3, 5, 7, 900), from_hmsm(3, 5, 7, 900)),
    ///            TimeDelta::Forwards(Duration::ZERO));
    /// assert_eq!(since(from_hmsm(3, 5, 7, 900), from_hmsm(3, 5, 7, 875)),
...
impl Add<Duration> for NaiveTime {
    ...
    fn add(self, rhs: Duration) -> NaiveTime {
        self.checked_overflowing_add(rhs).unwrap().0
    }
}
...
impl Sub<Duration> for NaiveTime {
    ...
    fn sub(self, rhs: Duration) -> NaiveTime {
        self.checked_overflowing_sub(rhs).unwrap().0
    }
}
...
/// let from_hmsm = NaiveTime::from_hms_milli;
///
/// assert_eq!(from_hmsm(3, 5, 7, 900) - from_hmsm(3, 5, 7, 900), TimeDelta::Forwards(Duration::ZERO));
