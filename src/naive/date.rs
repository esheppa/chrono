...
impl PartialEq<DaysDelta> for DaysDelta {
...
impl NaiveDate {
    ...
    pub fn days_since(self, rhs: NaiveDate) -> DaysDelta {
        ...
        if diff >= 0 {
            DaysDelta::Forwards(Days::new(diff.try_into().unwrap()))
        } else {
            DaysDelta::Backwards(Days::new(diff.abs().try_into().unwrap()))
        }
    }
...
/// Subtracts another `NaiveDate` from the current date.
/// Returns a `DaysDelta` of integral numbers.
///
/// This does not overflow or underflow at all,
...
///
/// ```
/// use chrono::{DaysDelta, NaiveDate, Days};
///
/// let from_ymd = NaiveDate::from_ymd;
///
/// assert_eq!(from_ymd(2014, 1, 1) - from_ymd(2014, 1, 1), DaysDelta::Forwards(Days::new(0)));
/// assert_eq!(from_ymd(2014, 1, 1) - from_ymd(2013, 12, 31), DaysDelta::Forwards(Days::new(1)));
/// assert_eq!(from_ymd(2014, 1, 1) - from_ymd(2014, 1, 2), DaysDelta::Backwards(Days::new(1)));
/// assert_eq!(from_ymd(2014, 1, 1) - from_ymd(2013, 9, 23), DaysDelta::Forwards(Days::new(100)));
/// assert_eq!(from_ymd(2014, 1, 1) - from_ymd(2013, 1, 1), DaysDelta::Forwards(Days::new(365)));
/// assert_eq!(from_ymd(2014, 1, 1) - from_ymd(2010, 1, 1), DaysDelta::Forwards(Days::new(365*4 + 1)));
/// assert_eq!(from_ymd(2014, 1, 1) - from_ymd(1614, 1, 1), DaysDelta::Forwards(Days::new(365*400 + 97)));
/// ```
impl Sub<NaiveDate> for NaiveDate {
    type Output = DaysDelta;

    #[inline]
    fn sub(self, rhs: NaiveDate) -> DaysDelta {
        self.days_since(rhs)
    }
}
...
mod tests {
    ...
    fn test_date_add_days() {
        ...
        check((0, 1, 1), Days::new(u32::try_from(MAX_DAYS_FROM_YEAR_0).unwrap() + 1), None);
    }
    ...
    fn test_day_iterator_limit() {
        assert_eq!(NaiveDate::from_ymd(262143, 12, 29).iter_days().take(4).count(), 3);
        assert_eq!(NaiveDate::from_ymd(-262144, 1, 3).iter_days().rev().take(4).count(), 3);
    }

    #[test]
    fn test_week_iterator_limit() {
        assert_eq!(NaiveDate::from_ymd(262143, 12, 12).iter_weeks().take(4).count(), 3);
        assert_eq!(NaiveDate::from_ymd(-262144, 1, 15).iter_weeks().rev().take(4).count(), 3);
    }
